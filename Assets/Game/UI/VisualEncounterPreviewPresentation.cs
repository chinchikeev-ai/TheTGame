...
public sealed class VisualEncounterPreviewPresentation : MonoBehaviour
{
    ...
    {
        if (FindFirstObjectByType<VisualEncounterPreviewPresentation>() == null)
            new GameObject("VisualEncounterPreviewPresentation").AddComponent<VisualEncounterPreviewPresentation>();
    }
    ...
    void Update()
    {
        if (!built)
        {
            ...
            if (hud == null) return;
            Transform encounter = hud.transform.Find("WaveStatus");
        ...
        int[] values =
        {
            EncounterRuntime.NextEncounterInfantryCount(spawner),
            EncounterRuntime.NextEncounterRunnerCount(spawner),
            EncounterRuntime.NextEncounterHeavyCount(spawner),
            EncounterRuntime.NextEncounterShieldCount(spawner),
            EncounterRuntime.NextEncounterArcherCount(spawner),
            EncounterRuntime.NextEncounterBossCount(spawner)
        };
    ...
    {
        row = new GameObject("EnemyEncounterCardRow");
        ...
        for (int i=0;i<cards.Length;i++)
        {
            GameObject card = new GameObject("EncounterCard_" + keys[i]);
