...
public sealed class CombatNotificationPresentation : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(gm==null){group.alpha=0f;return;}
        ...
        group.alpha=(gm.GameEnded||IsMenuBlocking())?0f:1f;
        ...
        if(bossActive&&!bossWasActive) Push(GameLanguage.T("MENELAUS HAS ENTERED THE BATTLE","МЕНЕЛАЙ ВСТУПИЛ В БОЙ"),TroyHudArt.Icon("boss"),7f);
    ...
    void Push(string text,Sprite icon,float duration)
    {
        entries.Insert(0,new Entry{text=text,icon=icon,expires=Time.unscaledTime+duration});
        while(entries.Count>3) entries.RemoveAt(entries.Count-1);
    }
    ...
    void Render()
    {
        for(int i=0;i<3;i++)
        {
            ...
            lines[i].gameObject.SetActive(visible); icons[i].gameObject.SetActive(visible);
            if(!visible) continue;
            lines[i].text=entries[i].text; icons[i].sprite=entries[i].icon; icons[i].color=Color.white;
        }
    }
