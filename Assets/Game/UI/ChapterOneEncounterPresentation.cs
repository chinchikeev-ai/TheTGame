...
public sealed class ChapterOneEncounterPresentation : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!bossWarningShown && EncounterRuntime.NextEncounterHasBoss(spawner) && !EncounterRuntime.EncounterActive(spawner))
        {
        ...
        if (EncounterRuntime.EncounterActive(spawner) && encounter > shownEncounter)
        {
    ...
    string EncounterTitle(int encounter)
    {
        ...
        if (encounter == 2) return GameLanguage.T("THE BEACHHEAD EXPANDS","ПЛАЦДАРМ РАСШИРЯЕТСЯ");
