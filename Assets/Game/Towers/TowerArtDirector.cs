    ...
    public static void Enhance(GameObject root, TowerType type)
    {
        ...
        switch (type)
        {
            case TowerType.MachineGun:
                ...
                AddProductionCrew(head != null ? head : art.transform,"Trojan_Archer",new Vector3(-.24f,.24f,-.02f),.60f,8f);
                AddProductionCrew(head != null ? head : art.transform,"Trojan_Archer",new Vector3(.24f,.24f,-.10f),.60f,-8f);
                break;
