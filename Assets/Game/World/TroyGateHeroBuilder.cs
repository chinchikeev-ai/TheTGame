...
public static class TroyGateHeroBuilder
{
    public static void Build()
    {
        ...
        for (int side = -1; side <= 1; side += 2)
        {
            Vector3 brazierPos = new Vector3(gateX - .75f, 1.15f, side * 2.25f);
            Part(root.transform, "Gate Brazier", PrimitiveType.Cylinder, brazierPos, new Vector3(.34f, .14f, .34f), bronze * .72f);
            GameObject flame = Part(root.transform, "Gate Flame", PrimitiveType.Sphere, brazierPos + new Vector3(0f, .34f, 0f), new Vector3(.22f, .42f, .22f), new Color(1f, .32f, .04f));
