...
8. `EDITOR_MENU.md` — canonical Unity Editor menu root, categories, migration and validation rules.
9. `TERMINOLOGY.md` — canonical player-facing naming.
10. `ENCOUNTER_TERMINOLOGY_MIGRATION.md` — Encounter-vs-Wave compatibility boundary.
11. `AI_PIPELINE.md` — validation and CI truthfulness rules.
12. `MODEL_ART_INVENTORY.md` — authoritative art/model completion status.
13. `ART_BIBLE.md` — single canonical creative + visual direction authority.
14. `UNITY_ROADMAP.md` — what should be built next.
15. `GDD.md` — canonical game-design target.
...
4. **`EDITOR_MENU.md`** — Unity Editor command naming and placement contract.
5. **`TERMINOLOGY.md` / `ENCOUNTER_TERMINOLOGY_MIGRATION.md`** — canonical naming and compatibility boundaries.
6. **`MODEL_ART_INVENTORY.md`** — production-art completion truth.
7. **`ART_BIBLE.md`** — single creative/visual direction authority.
8. **`GDD.md`** — intended game design.
9. **`UNITY_ROADMAP.md`** — planned implementation order.
10. Design-detail documents and audits — supporting reference only.
...
- `EDITOR_MENU.md` — single `The Troy Game/` Unity Editor menu root, allowed categories, legacy compatibility and validation contract.
...
7. One canonical Unity Editor menu contract: `EDITOR_MENU.md`; project-owned commands use `The Troy Game/` only.
8. Canonical runtime combat terminology is Encounter; do not create new Wave-prefixed gameplay APIs or `WaveData`.
9. Canonical player-facing divine terminology comes from `TERMINOLOGY.md`; runtime legacy names do not override it.
10. Do not create a second document for the same authority/scope.
11. Delete obsolete implementation plans instead of leaving competing instructions.
12. Merge overlapping references when one canonical document can carry the information cleanly.
13. Update status/inventory in the same change that materially changes implementation/art state.
14. Future concepts must be explicitly labeled as future/design-only.
15. Never claim Unity compile/tests/build are green unless those checks actually ran and passed.
16. Do not promote production art that conflicts with `ART_BIBLE.md` even if it is technically complete.
