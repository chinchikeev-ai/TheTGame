...
- В матрице отдельно проверить: guidance ниже ресурсов, уведомления не перекрывают Гектора, next-encounter cards не конфликтуют с прогрессом/скоростью, Patron не конфликтует с BossHUD/Divine Power, enemy inspector не конфликтует с selected-defense card.
...
- Клик по врагу открывает его характеристики с портретом; инспектор скрывается за паузой/настройками и возвращается корректно.
